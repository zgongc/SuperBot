...
Açıklama:
    Chandelier Exit - ATR tabanlı trailing stop göstergesi
    Pozisyon çıkış noktalarını belirlemek için kullanılır
    Yükseliş ve düşüş trendlerinde farklı stop seviyeleri oluşturur
...
class ChandelierExit(BaseIndicator):
    ...
    ATR tabanlı trailing stop seviyeleri oluşturur.
    Long ve short pozisyonlar için farklı çıkış seviyeleri sağlar.

    Args:
        period: Highest/Lowest ve ATR periyodu (varsayılan: 22)
    ...
    def get_required_periods(self) -> int:
        ...
        return self.period + 1  # TR hesabı için ekstra mum
    ...
    def calculate(self, data: pd.DataFrame) -> IndicatorResult:
        ...
        # Fiyatın stop seviyelerine olan mesafesi (yüzde)
        if current_price > 0:
        ...
        return IndicatorResult(
            ...
            strength=min(abs(long_distance) * 2, 100),  # 0-100 arası normalize et
            metadata={
                'period': self.period,
    ...
    def calculate_batch(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        ⚡ VECTORIZED batch Chandelier Exit calculation - BACKTEST için
    ...
    def get_signal(self, price: float, long_stop: float, short_stop: float) -> SignalType:
        """
        Fiyat ve stop seviyelerinden sinyal üret

        Args:
            price: Güncel fiyat
            long_stop: Long stop seviyesi
            short_stop: Short stop seviyesi

        Returns:
            SignalType: BUY, SELL veya HOLD
        """
        # Fiyat long stop'un altına düştü: long çıkışı
        if price < long_stop:
            return SignalType.SELL
        # Fiyat short stop'un üstüne çıktı: short çıkışı
    ...
    def get_trend(self, price: float, long_stop: float, short_stop: float) -> TrendDirection:
        """
        Fiyat ve stop seviyelerinden trend belirle
        ...
        """
        # Fiyat long stop üstünde: yükseliş trendi
        if price > long_stop:
            return TrendDirection.UP
        # Fiyat short stop altında: düşüş trendi
        elif price < short_stop:
...
if __name__ == "__main__":
    """Chandelier Exit indikatör testi"""
    ...
    print(f"   [OK] Oluşturuldu: {ce}")
    print(f"   [OK] Kategori: {ce.category.value}")
    print(f"   [OK] Tip: {ce.indicator_type.value}")
    print(f"   [OK] Gerekli periyot: {ce.get_required_periods()}")
    ...
    # Test 4: Yükseliş trendi simülasyonu
    print("\n5. Yükseliş trendi testi...")
    uptrend_prices = [100]
    for i in range(34):
        change = abs(np.random.randn()) * 1.5  # Sadece pozitif
    ...
    print(f"   [OK] Yükseliş Trend: {result.trend.name}")
    print(f"   [OK] Sinyal: {result.signal.value}")
    print(f"   [OK] Fiyat: {result.metadata['price']:.2f}, Long Stop: {result.value['long_stop']:.2f}")

    # Test 5: Düşüş trendi simülasyonu
    print("\n6. Düşüş trendi testi...")
    downtrend_prices = [100]
    for i in range(34):
        change = abs(np.random.randn()) * 1.5  # Sadece negatif
    ...
    print(f"   [OK] Düşüş Trend: {result.trend.name}")
    print(f"   [OK] Sinyal: {result.signal.value}")
    print(f"   [OK] Fiyat: {result.metadata['price']:.2f}, Short Stop: {result.value['short_stop']:.2f}")
