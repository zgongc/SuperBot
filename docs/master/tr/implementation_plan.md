...
AI destekli, çok exchange'li crypto trading platformu. Modüler mimari ile Trading, Backtest, AI ve WebUI modüllerinin bağımsız çalışması.

### Teknoloji Stack
- **Python:** 3.12
- **Exchanges:** Binance (python-binance), Kucoin/Bitget/OKX/Bybit (ccxt)
- **Database:** SQLite/PostgreSQL (config seçilebilir)
- **Cache:** Memory/Redis (config seçilebilir)
- **Queue:** Memory/RabbitMQ (config seçilebilir)
- **AI:** FastAPI + XGBoost
- **WebUI:** Flask

### Trading Modları
1. **Paper:** Gerçek API + fake orders (simulation)
...
│   │   └── order_executor.py
│   ├── indicators/                  # Hazır (momentum, trend, volume, etc.)
│   ├── managers/
│   │   ├── order_manager.py
│   │   ├── position_manager.py
│   │   ├── risk_manager.py
...
- 4 YAML dosyasını yükle (main, infrastructure, connectors, daemon)
- Environment variable substitution (${REDIS_HOST} gibi)
- Config validation ve default values
- get_config() singleton pattern
- Hot-reload support (file_watcher ile entegre)

#### ✅ core/logger_engine.py
- JSON/text format desteği (config'dan seçilebilir)
- Türkçe mesajlar + emoji standardı
- Modül bazlı logger instance'ları
- get_logger(module_name) factory
- Log rotation ve compression
- Console + dosya çıktısı

#### ✅ core/event_bus.py
- Memory/Redis backend desteği (config'dan seçilir)
- Pub/sub pattern implementasyonu
- Event broadcasting sistemi (trade.opened, order.filled, etc.)
- Event filtreleme ve priority levels
- Dead letter queue desteği

#### ✅ core/cache_manager.py
- Memory/Redis backend switcher
- TTL desteği ve eviction policies (LRU, LFU, FIFO)
- Key prefix management
- get/set/delete/exists operasyonları
- Batch operations desteği

#### ✅ core/queue_manager.py
- Memory/RabbitMQ backend switcher
- Task queue (add_job, process_job)
- Priority queue desteği (high, medium, low)
- Retry logic + dead letter queue
- Worker pool yönetimi

#### 🔨 core/rate_limiter.py
- Exchange API rate limit yönetimi (exchange bazlı)
- Request throttling ve queuing
- Weight tracking (Binance için)
- Sliding window algoritması
- Rate limit aşım durumunda handling
...
- Module cleanup koordinasyonu
- State persistence (kapatmadan önce)
- Timeout yönetimi
- Emergency shutdown
...
- Exponential backoff
- Max attempts konfigürasyonu
- Error type filtering
- Callback hooks (on_retry, on_failure)

#### ✅ core/thread_pool_manager.py
- Modül bazlı thread pool allocation
- ThreadPoolExecutor wrapper
- Dynamic pool resizing
- Task priority desteği

#### ✅ core/process_manager.py
- Module lifecycle (start/stop/restart)
- Process monitoring
- Health check koordinasyonu
- Crash durumunda auto-restart

#### ✅ core/filewatcher_engine.py
- Config dosya monitoring (watchdog)
...
- Trade model (entry, exit, PnL, strateji bilgisi)
- Order model (order tracking, state management)
- Position model (açık pozisyonlar)
- Signal model (sinyal geçmişi, outcome)
- Balance model (portföy snapshot, zaman serisi)
- StrategyPerformance model (strateji metrikleri)
- ExchangeAccount model (WebUI portfolio tracking için)
...
- Binance, Kucoin, Bitget, OKX, Bybit desteği
...
- Historical data CRUD operasyonları
- Data registry (hangi sembol/timeframe mevcut)
- Gap detection ve filling
- Data versioning
- Query interface

#### 🔨 components/data/market_data_recorder.py
- Live trading sırasında tick data recording
- WebSocket stream → database/parquet
- Replay için veri toplama
- Buffer management
- Archive scheduler
...
- Exchange'den sembol bilgisi çekme
...
- CCXT üzerinden Kucoin
- Futures + Spot
- API credential yönetimi

#### ✅ components/exchanges/bitget_api.py
- CCXT üzerinden Bitget
- Futures + Spot

#### 🔨 components/exchanges/okx_api.py
- CCXT üzerinden OKX (Phase 2)
- Passphrase requirement

#### 🔨 components/exchanges/bybit_api.py
- CCXT üzerinden Bybit (Phase 2)
...
#### ✅ components/managers/position_manager.py (Kısmen hazır)
...
#### ✅ components/managers/risk_manager.py (Kısmen hazır)
...
- Gerçek API + fake orders
- Real-time simulation
- Slippage ve latency simulation
- Order fill simulation
...
#### ✅ modules/ai/server/main.py (Kısmen hazır)
- FastAPI server
- Uvicorn runner
- Model loading
...
#### ✅ modules/ai/ai_client.py (Kısmen hazır)
- AI server client
- Prediction requests
- Timeout handling
- Fallback logic
...
- Backtest sonuçlarından label extraction
...
- IPC client (daemon ile iletişim)
- Command routing
- Output formatting
- Interactive mode (opsiyonel)
...
- Data alignment ve synchronization

### Infrastructure Flexibility
Config'dan seçilebilir backend'ler:
...
- Backtest + live trading tek template
...
**Detaylar:** `docs/master/rules.md`

### Dosya Standartları
- Header/footer zorunlu (şablon rules.md'de)
- Type hints + docstrings
- Test bölümü (`if __name__ == "__main__"`)
...
### ✅ Hazır
- Core infrastructure (12/13 dosya)
- Config dosyaları (4/4)
- Exchange base + Binance/Kucoin/Bitget
- Indicators (tümü)
- Strategy template sistemi
- Position/Risk managers (kısmen)
...
- Trading module (4 mod: paper/demo/live/replay)
